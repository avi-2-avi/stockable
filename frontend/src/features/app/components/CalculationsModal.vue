<template>
    <Modal title="How is CPI Calculated?" @close="$emit('close')">
        <div class="text-sm space-y-4 overflow-y-auto max-h-[70vh] px-2">
            <p>
            ...
            <h4 class="font-semibold">Action Impact Score (AIS)</h4>
            <p>The AIS assesses the impact of an analyst action on stock price movement:</p>
            <p class="italic">Formula: AIS = Weight<sub>action</sub> × (Target<sub>to</sub> - Target<sub>from</sub>)</p>
            ...
            <p>Measures the effect of a rating upgrade or downgrade.</p>
            <p class="italic">Formula: RCI = Rating<sub>to</sub> - Rating<sub>from</sub></p>

            <h4 class="font-semibold text-lg">RCI Matrix Table:</h4>
            ...
            <p>Determines the percentage change in target price:</p>
            <p class="italic">TAP = min(max((Target<sub>to</sub> - Target<sub>from</sub>) / Target<sub>from</sub> × 100,
                -100), 100)</p>
            <p>If TAP &lt; 0, a penalty factor of 1.5 is applied.</p>

            <h4 class="font-semibold">Combined Prediction Index (CPI)</h4>
            <p>The final CPI is calculated using weighted averages:</p>
            <p class="italic">CPI = 0.3 × AIS + 0.35 × RCI + 0.35 × TAP</p>
            <p>The raw CPI is then normalized to a 0-100 scale:</p>
            <p class="italic">Normalized CPI = ((CPI - CPI<sub>min</sub>) / (CPI<sub>max</sub> - CPI<sub>min</sub>)) ×
                100</p>
            <p class="mt-2">The higher the CPI, the stronger the buy signal. A lower CPI suggests caution.</p>
        </div>
